...
```mermaid
graph TD
    A[Student Mobile/Web] --> B[API Gateway]
    C[Company Portal] --> B
    D[Institution Portal] --> B
    B --> E[Application Services]
    E --> F[Neural Engine Microservice]
    F --> G[Vector DB / PostgreSQL]
    E --> H[Blockchain Ledger]
    E --> I[M-Pesa Integration]
    F --> J[HuggingFace LLMs]
```
